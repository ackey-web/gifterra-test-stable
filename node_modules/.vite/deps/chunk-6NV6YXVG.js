...
// node_modules/.pnpm/@thirdweb-dev+sdk@4.0.99_@types+react-dom@18.3.7_@types+react@18.3.26__@types+react@18._4ecbbc39539d7059b1b8ad321c3f738a/node_modules/@thirdweb-dev/sdk/dist/deployContractDeterministic-7823c15c.browser.esm.js
init_lib();
...
// node_modules/.pnpm/@thirdweb-dev+sdk@4.0.99_@types+react-dom@18.3.7_@types+react@18.3.26__@types+react@18._4ecbbc39539d7059b1b8ad321c3f738a/node_modules/@thirdweb-dev/sdk/dist/airdrop-erc1155-f9016b81.browser.esm.js
init_lib();
...
var SignerPermissionsSchema = (() => external_exports.object({
  startDate: StartDateSchema,
  expirationDate: EndDateSchema,
  nativeTokenLimitPerTransaction: AmountSchema.default(0),
  approvedCallTargets: external_exports.union([external_exports.array(AddressOrEnsSchema), external_exports.literal("*")])
}))();
var PermissionSnapshotSchema = (() => external_exports.array(external_exports.object({
  signer: AddressOrEnsSchema,
  makeAdmin: external_exports.boolean(),
...
var AccountPermissions = class {
   ...
   * @twfeature AccountPermissions
   */
  grantAdminPermissions = buildTransactionFunction(async (signerAddress) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
   ...
   * @twfeature AccountPermissions
   */
  revokeAdminPermissions = buildTransactionFunction(async (signerAddress) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
   ...
   * @twfeature AccountPermissions
   */
  grantPermissions = buildTransactionFunction(async (signerAddress, permissions) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
   ...
   * @twfeature AccountPermissions
   */
  updatePermissions = buildTransactionFunction(async (signerAddress, permissions) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
    const resolvedPermissions = await SignerPermissionsSchema.parseAsync(permissions);
   ...
   * @twfeature AccountPermissions
   */
  revokeAccess = buildTransactionFunction(async (signerAddress) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
   ...
   * @twfeature AccountPermissions
   */
  approveTargetForSigner = buildTransactionFunction(async (signerAddress, target) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
   ...
   * @twfeature AccountPermissions
   */
  disapproveTargetForSigner = buildTransactionFunction(async (signerAddress, target) => {
    const resolvedSignerAddress = await resolveAddress(signerAddress);
    const resolvedTarget = await resolveAddress(target);
    if (await this.isAdmin(resolvedSignerAddress)) {
      throw new Error("Signer is already an admin. Cannot approve targets for an admin.");
   ...
   * @twfeature AccountPermissions
   */
  resetAllPermissions = buildTransactionFunction(async (permissionSnapshot) => {
...
var AccountFactory = class {
   ...
   * @twfeature AccountFactory
   */
  createAccount = buildTransactionFunction(async (accountAdmin, extraData) => {
...
var ExtensionManager = class {
   ...
   */
  add = buildTransactionFunction(async (inputArgs) => {
    return Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "addExtension",
   ...
   */
  addDeployed = buildTransactionFunction(async (inputArgs) => {
   ...
   */
  addPublished = buildTransactionFunction(async (inputArgs) => {
  ...
  replace = buildTransactionFunction(async (inputArgs) => {
    return Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
  ...
  replaceDeployed = buildTransactionFunction(async (inputArgs) => {
    let extensionAbi = inputArgs.extensionAbi;
    if (!extensionAbi) {
      const metadata = await fetchContractMetadataFromAddress(inputArgs.extensionAddress, this.contractWrapper.getProvider(), this.contractWrapper.storage, this.contractWrapper.options);
      extensionAbi = metadata.abi;
  ...
  replacePublished = buildTransactionFunction(async (inputArgs) => {
    const version = inputArgs.version || "latest";
    const {
      deployedExtensionAddress,
      extensionMetadata
  ...
  remove = buildTransactionFunction(async (inputArgs) => {
...
var Airdrop20 = class {
   ...
   */
  drop = buildTransactionFunction(async (tokenAddress, tokenOwner, contents) => {
    return Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "airdropERC20",
...
//# sourceMappingURL=chunk-6NV6YXVG.js.map
