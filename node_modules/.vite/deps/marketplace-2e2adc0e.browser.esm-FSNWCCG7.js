...
// node_modules/.pnpm/@thirdweb-dev+sdk@4.0.99_@types+react-dom@18.3.7_@types+react@18.3.26__@types+react@18._4ecbbc39539d7059b1b8ad321c3f738a/node_modules/@thirdweb-dev/sdk/dist/marketplace-2e2adc0e.browser.esm.js
init_lib();
...
var MarketplaceAuction = class {
   ...
   * ```
   */
  buyoutListing = buildTransactionFunction(async (listingId) => {
    const listing = await this.validateListing(BigNumber.from(listingId));
   ...
   * ```
   */
  makeBid = buildTransactionFunction(async (listingId, pricePerToken) => {
    const listing = await this.validateListing(BigNumber.from(listingId));
   ...
   */
  closeListing = buildTransactionFunction(async (listingId, closeFor) => {
    if (!closeFor) {
   ...
   * @param listingId - the auction  listing ud to close
   */
  executeSale = buildTransactionFunction(async (listingId) => {
    const listing = await this.validateListing(BigNumber.from(listingId));
    try {
      const winningBid = await this.getWinningBid(listingId);
   ...
   */
  updateListing = buildTransactionFunction(async (listing) => {
    const tx = Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
...
var MarketplaceDirect = class {
   ...
   * ```
   */
  makeOffer = buildTransactionFunction(async (listingId, quantityDesired, currencyContractAddress, pricePerToken, expirationDate) => {
   ...
   * ```
   */
  acceptOffer = buildTransactionFunction(async (listingId, addressOfOfferor) => {
    await this.validateListing(BigNumber.from(listingId));
   ...
   */
  buyoutListing = buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
    const listing = await this.validateListing(BigNumber.from(listingId));
    const {
      valid,
      error
...
var Marketplace = class _Marketplace {
   ...
   * ```
   */
  makeOffer = buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
    const listing = await this.contractWrapper.read("listings", [listingId]);
    if (listing.listingId.toString() !== listingId.toString()) {
      throw new ListingNotFoundError(this.getAddress(), listingId.toString());
    }
   ...
   */
  setBidBufferBps = buildTransactionFunction(async (bufferBps) => {
   ...
   */
  setTimeBufferInSeconds = buildTransactionFunction(async (bufferInSeconds) => {
    await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
   ...
   */
  allowListingFromSpecificAssetOnly = buildTransactionFunction(async (contractAddress) => {
    const encoded = [];
...
//# sourceMappingURL=marketplace-2e2adc0e.browser.esm-FSNWCCG7.js.map
