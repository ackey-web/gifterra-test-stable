...
// node_modules/.pnpm/@thirdweb-dev+sdk@4.0.99_@types+react-dom@18.3.7_@types+react@18.3.26__@types+react@18._4ecbbc39539d7059b1b8ad321c3f738a/node_modules/@thirdweb-dev/sdk/dist/marketplacev3-offers-ce1f096b.browser.esm.js
init_lib();
var DirectListingInputParamsSchema = (() => external_exports.object({
  /**
   ...
   */
  isReservedListing: external_exports.boolean().default(false)
}))();
...
var MarketplaceV3DirectListings = class {
   ...
   * @twfeature DirectListings
   */
  createListing = buildTransactionFunction(async (listing) => {
   ...
   * ```
   */
  createListingsBatch = buildTransactionFunction(async (listings) => {
   ...
   * @twfeature DirectListings
   */
  updateListing = buildTransactionFunction(async (listingId, listing) => {
    const parsedListing = await DirectListingInputParamsSchema.parseAsync(listing);
    await handleTokenApproval(this.contractWrapper, this.getAddress(), parsedListing.assetContractAddress, parsedListing.tokenId, await this.contractWrapper.getSignerAddress());
    const normalizedPricePerToken = await normalizePriceValue(this.contractWrapper.getProvider(), parsedListing.pricePerToken, parsedListing.currencyContractAddress);
    const tx = Transaction.fromContractWrapper({
   ...
   * @twfeature DirectListings
   */
  cancelListing = buildTransactionFunction(async (listingId) => {
    const tx = Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
   ...
   * @twfeature DirectListings
   */
  buyFromListing = buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
    if (receiver) {
   ...
   * @twfeature DirectListings
   */
  approveBuyerForReservedListing = buildTransactionFunction(async (listingId, buyer) => {
   ...
   * @param buyer - Address of buyer being approved
   */
  revokeBuyerApprovalForReservedListing = buildTransactionFunction(async (listingId, buyer) => {
    const isApproved = await this.isBuyerApprovedForListing(listingId, buyer);
    if (isApproved) {
      const tx = Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
   ...
   * @twfeature DirectListings
   */
  approveCurrencyForListing = buildTransactionFunction(async (listingId, currencyContractAddress, pricePerTokenInCurrency) => {
    const listing = await this.validateListing(BigNumber.from(listingId));
    const resolvedCurrencyAddress = await resolveAddress(currencyContractAddress);
    if (resolvedCurrencyAddress === listing.currencyContractAddress) {
   ...
   * @twfeature DirectListings
   */
  revokeCurrencyApprovalForListing = buildTransactionFunction(async (listingId, currencyContractAddress) => {
    const listing = await this.validateListing(BigNumber.from(listingId));
    const resolvedCurrencyAddress = await resolveAddress(currencyContractAddress);
    if (resolvedCurrencyAddress === listing.currencyContractAddress) {
  ...
  async isStillValidListing(listing, quantity) {
    ...
    const ERC165Abi = (await import("./IERC165-MONVI52P.js")).default;
    const erc165 = new Contract(listing.assetContractAddress, ERC165Abi, provider);
    const isERC721 = await erc165.supportsInterface(InterfaceId_IERC721);
    const isERC1155 = await erc165.supportsInterface(InterfaceId_IERC1155);
    if (isERC721) {
      const ERC721Abi = (await import("./IERC721-Q64TRPEO.js")).default;
      ...
      const ERC1155Abi = (await import("./IERC1155-CSJXTZOE.js")).default;
...
var EnglishAuctionInputParamsSchema = (() => external_exports.object({
  /**
...
var MarketplaceV3EnglishAuctions = class {
   ...
   * @twfeature EnglishAuctions
   */
  createAuction = buildTransactionFunction(async (auction) => {
   ...
   * ```
   */
  createAuctionsBatch = buildTransactionFunction(async (listings) => {
   ...
   * @twfeature EnglishAuctions
   */
  buyoutAuction = buildTransactionFunction(async (auctionId) => {
    const auction = await this.validateAuction(BigNumber.from(auctionId));
   ...
   * @twfeature EnglishAuctions
   */
  makeBid = buildTransactionFunction(async (auctionId, bidAmount) => {
    const auction = await this.validateAuction(BigNumber.from(auctionId));
   ...
   * @twfeature EnglishAuctions
   */
  cancelAuction = buildTransactionFunction(async (auctionId) => {
    const winningBid = await this.getWinningBid(auctionId);
    if (winningBid) {
   ...
   * @twfeature EnglishAuctions
   */
  closeAuctionForBidder = buildTransactionFunction(async (auctionId, closeFor) => {
    if (!closeFor) {
   ...
   * @twfeature EnglishAuctions
   */
  closeAuctionForSeller = buildTransactionFunction(async (auctionId) => {
    const auction = await this.validateAuction(BigNumber.from(auctionId));
    try {
      const tx = Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
   ...
   * @twfeature EnglishAuctions
   */
  executeSale = buildTransactionFunction(async (auctionId) => {
    const auction = await this.validateAuction(BigNumber.from(auctionId));
    try {
      const winningBid = await this.getWinningBid(auctionId);
...
var OfferInputParamsSchema = (() => external_exports.object({
  /**
...
var MarketplaceV3Offers = class {
   ...
   * @twfeature Offers
   */
  makeOffer = buildTransactionFunction(async (offer) => {
   ...
   * @twfeature Offers
   */
  cancelOffer = buildTransactionFunction(async (offerId) => {
    return Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "cancelOffer",
      args: [offerId]
   ...
   * @twfeature Offers
   */
  acceptOffer = buildTransactionFunction(async (offerId) => {
...
//# sourceMappingURL=chunk-VISWZ4NV.js.map
