  ...
  const RewardUIManagementPage = () => {
    ...
    const loadDailyRewardData = async () => {
      ...
      try {
        ...
        const [dailyAmount, tokenAddress] = await Promise.all([
        ...
        setRewardTokenAddress(tokenAddress);
        
        // トークン残高も取得
        await loadTokenBalances(provider, tokenAddress);
      } catch (error: any) {
    ...
    // トークン残高取得
    const loadTokenBalances = async (provider: ethers.providers.Web3Provider, tokenAddress: string) => {
      try {
        ...
        setUserTokenBalance(ethers.utils.formatUnits(userBalance, 18));
        setContractTokenBalance(ethers.utils.formatUnits(contractBalance, 18));
      } catch (error: any) {
    ...
    // トークンチャージ機能
    const chargeRewardToken = async () => {
      if (!address || !isAdmin || !chargeAmount || !rewardTokenAddress) return;
      ...
      try {
        ...
        const parsedAmount = ethers.utils.parseUnits(chargeAmount, 18);
        
        // Step 1: Approve
        showToast("承認トランザクションを送信中...", "success");
        const approveTx = await tokenContract.approve(CONTRACT_ADDRESS, parsedAmount);
        await approveTx.wait();
        
        // Step 2: Transfer
        showToast("転送トランザクションを送信中...", "success");
        const transferTx = await tokenContract.transfer(CONTRACT_ADDRESS, parsedAmount);
        await transferTx.wait();
        
        // Step 3: Update balances
        await loadTokenBalances(provider, rewardTokenAddress);
        
        setChargeAmount("");
        showToast(`✅ ${chargeAmount} ${TOKEN.SYMBOL} をチャージしました！`, "success");
      } catch (error: any) {
        console.error("Failed to charge token:", error);
        showToast(`チャージエラー: ${error?.reason || error?.message || "不明なエラー"}`, "error");
      } finally {
        setIsCharging(false);
      }
    };
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          background: "rgba(255,255,255,.06)",
          borderRadius: 12,
          border: isAdmin ? "2px solid #2563eb" : "1px solid rgba(255,255,255,.1)"
        }}>
          <h3 style={{ margin: "0 0 16px 0", fontSize: 18, fontWeight: 700 }}>
            🪙 リワードトークンチャージ
          </h3>
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ opacity: 0.8, marginBottom: 4 }}>あなたの残高:</div>
                  <div style={{ fontFamily: "monospace", fontWeight: 600, fontSize: 16 }}>
                    {parseFloat(userTokenBalance).toFixed(4)} {TOKEN.SYMBOL}
                  </div>
                </div>
                <div style={{
                  padding: 12,
                  ...
                  <div style={{ opacity: 0.8, marginBottom: 4 }}>コントラクト残高:</div>
                  <div style={{ fontFamily: "monospace", fontWeight: 600, fontSize: 16 }}>
                    {parseFloat(contractTokenBalance).toFixed(4)} {TOKEN.SYMBOL}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "end" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.0001"
                    placeholder={`例: 1000 (最大: ${parseFloat(userTokenBalance).toFixed(4)})`}
                    disabled={isCharging}
                    style={{
                      width: "100%",
                      padding: 8,
                ...
                <button
                  onClick={chargeRewardToken}
                  disabled={isCharging || !chargeAmount || parseFloat(chargeAmount) <= 0 || parseFloat(chargeAmount) > parseFloat(userTokenBalance)}
                  style={{
                    background: isCharging ? "#6b7280" : "#2563eb",
                    color: "#fff",
                    border: "none",
                    borderRadius: 8,
                    padding: "10px 20px",
                    ...
                    opacity: isCharging || !chargeAmount || parseFloat(chargeAmount) <= 0 || parseFloat(chargeAmount) > parseFloat(userTokenBalance) ? 0.7 : 1,
                    whiteSpace: "nowrap"
                  }}
                >
                  {isCharging ? "送信中..." : "💰 チャージ実行"}
                </button>
              </div>
              
              <div style={{ marginTop: 8, fontSize: 12, opacity: 0.7 }}>
                💡 チャージは approve → transfer の2段階で実行されます
              </div>
            </div>
          )}
        </div>
